...
각 단계마다 같은 부하를 주고 정합성과 성능이 어떻게 바뀌는지 기록한다.

## 벤치마크 Summary

각 단계에서 동일 조건으로 5회 반복 측정했다. 처리량·p95 같은 성능 지표는 median으로, oversell 같은 정합성 지표는 median이 아니라 5회 중 발생 횟수로 적었다.
...
| -------------- | ------------- | ---------------- | ------- |
| 1. Naive       | 5/5회         | 4~67건           | false   |
| 2. DB Atomic   | 0/5회         | -                | true    |
| 3. Redis       | 0/5회         | -                | true    |
| 4. Async write | 0/5회         | -                | true    |

> Hotspot은 latency를 제외했다. 총 500요청이 약 0.1초 만에 끝나 표본이 작아 p95가 노이즈에 지배되기 때문이다. 실제로 단계 간 median 차이보다 run 간 편차가 더 크다. Hotspot에서 볼 것은 정합성이 깨졌는지 여부다.

### Spread

| 단계           | 총 요청 | 처리량 (req/s) | p95 (total) | p95 (200 OK) | oversell 발생      |
| -------------- | ------- | -------------- | ----------- | ------------ | ------------------ |
| 1. Naive       | 341,682 | 10,262         | 83.2ms      | 8.8ms        | 3/5회 (최대 3좌석) |
| 2. DB Atomic   | 336,825 | 10,116         | 84.1ms      | 9.2ms        | 0/5회              |
| 3. Redis       | 508,549 | 15,480         | 51.5ms      | 9.7ms        | 0/5회              |
| 4. Async write | 517,581 | 15,559         | 51.7ms      | 1.7ms        | 0/5회              |

단계별 상세 분석은 아래 [Naive](#1-naive-implementation-260526) ~ [Async write](#4-asynchronous-write-260702) 섹션에서 다룬다.
...
seatCount: 1
oversell: 5회 모두 발생
bookingCount: 4~67 (run마다 크게 요동)
```

500개 동시 요청이 좌석 하나에 몰리자 5회 모두 oversell이 터졌고, 한 좌석에 쌓인 중복은 run마다 4건에서 67건까지 크게 튀었다. 대부분은 정상적으로 409 Conflict를 받았지만, 매번 수~수십 건이 동시에 검사를 통과해버린 것이다.

**Spread**

```
seatCount: 100
http_reqs: 341,682 (약 10,262 req/s)
http_req_duration p95: 83.2ms
oversell: 5회 중 3회 발생 (최대 3개 좌석, 좌석별 2건 중복)
```

경합을 100개 좌석으로 분산시키자 oversell이 훨씬 드물어졌다. Hotspot에서는 500요청만으로도 매 run 한 좌석에 중복이 터졌지만, spread에서는 34만 건이 넘는 요청에서도 5회 중 3회만, 그마저 최대 3개 좌석에서 2건씩 중복되는 데 그쳤다. 경합이 한 점에 몰리느냐 흩어지느냐에 따라 발현 빈도가 크게 달라진다는 걸 보여준다.

이 성능 수치는 이후 단계의 기준선이다. 2단계에서 원자적 연산을 적용해 정합성을 지키고, 3/4단계에서 성능을 올리며 벤치마크가 어떻게 달라지는지 분석할 것이다.

<br>

## 2. DB-level atomicity (260527)

1단계의 문제는 검사와 점유가 두 번의 DB 왕복으로 쪼개져 있다는 것이었다. 그 사이의 틈이 race condition의 발생 지점이다. 틈을 없애려면 검사와 점유를 하나의 연산으로 합치면 된다.

방법은 사실 아주 단순하다. 검사 조건을 update의 필터 안으로 밀어넣으면 MongoDB가 알아서 원자적으로 처리해준다.
...
500개의 동시 요청 중 정확히 1건만 좌석을 잡고 나머지 499건은 의도된대로 409를 받았다. 정합성이 보장되었다.

**Spread**

```
seatCount: 100
http_reqs: 336,825 (약 10,116 req/s)
http_req_duration p95: 84.1ms
oversoldCount: 0
```

100개 좌석에 분산시킨 33만 건의 요청에서도 oversell이 사라졌다. 좌석 100개에 정확히 100건의 booking이 박혔다.

흥미로운 건 성능이다. 정합성을 얻으면 성능을 잃는다는 trade-off의 직관과 달리, spread의 처리량과 지연은 1단계와 거의 동일했다(10,262 - 10,116 req/s, p95 83.2 - 84.1ms). 사실상 측정 노이즈 범위 안이라고 할 수 있다. 분산 부하에서는 좌석마다 경합이 흩어져 있어 조건부 update의 직렬화 비용이 잘 드러나지 않기 때문이다. 정합성을 사실상 공짜로 얻었다.

정합성을 얻었으니, 이후 단계에서는 성능을 끌어올리는 데 집중한다.

<br>

## 3. Redis (260531)

2단계에서 정합성을 얻었지만 근본적인 문제가 하나 남았다. Spread에서 매진 이후 들어오는 34만 건의 요청 중 99.97%의 실패 요청이 전부 DB까지 내려가 조건부 update를 날리고 이선좌(MatchedCount 0) 에러를 받아간다. 티켓팅 서버의 진짜 부하는 표를 잡는 소수가 아니라, 매진인데도 새로고침하며 들이치는 다수다. 그 다수를 DB까지 보내지 않고 Redis를 사용해 메모리에서 거르는 게 이 단계의 목표다.
...
MongoDB에 저장하는 메서드는 1단계의 것을 그대로 썼는데, Redis에서 유효성 판단을 해주므로 정합성이 유지됐다.

**Spread**

```
seatCount: 100
http_reqs: 508,549 (약 15,480 req/s)
http_req_duration p95: 51.5ms
oversoldCount: 0
```

여기서 효과가 드러난다. 처리량이 10,116에서 15,480 req/s로 뛰고 p95가 84.1ms에서 51.5ms로 크게 줄었다. 이선좌 요청 50만 건이 빠른 Redis에서 잘려나가고 MongoDB는 좌석을 실제로 잡은 100명만 봤기 때문이다. 매진 트래픽을 메모리에서 거르는 것만으로 정합성을 유지한 채 성능까지 압도적으로 끌어올렸다.

<br>

## 4. Asynchronous write (260702)

3단계 끝에 남은 숙제는 write가 아직 동기라는 것이었다. 좌석을 잡은 요청은 Redis claim으로 이미 승부가 났는데도, seat update와 booking insert라는 Mongo write 두 방을 요청 안에서 끝내고서야 200을 받는다. 정합성 방어선은 Redis에 있으니 write는 요청 경로에서 떼어내 언제 하든 상관없어야 한다. 이걸 worker pool로 비동기 처리해 성능을 끌어올리는 게 4단계다.

승자 판정(SetNX)이 끝나면 job을 buffered channel에 넣고 곧바로 리턴한다. 채널 뒤에 붙은 worker 고루틴들이 Mongo write를 비동기로 처리한다.
...
write가 비동기가 되면서 측정에 구멍이 하나 생긴다. verify는 booking을 집계해 정합성을 판정하는데, k6가 끝난 직후 큐에 write가 남아 있으면 booking이 실제보다 적게 잡혀 오탐이 난다. 그래서 큐 depth를 노출하는 `/health/queue`를 두고, reproduce 스크립트가 depth 0을 확인한 뒤에 verify를 때리도록 했다. 이건 새 성능 지표가 아니라 verify 타이밍만 보정하는 drain 확인용이라, 1~3단계의 측정 기준(hotspot 정합성 + spread 처리량/p95)은 그대로 유지된다.
...
승자 요청의 처리 과정만 바꾼 것이므로 정합성은 당연히 유지된다.

**Spread**

```
seatCount: 100
http_reqs: 517,581 (약 15,559 req/s)
http_req_duration p95: 51.7ms
totalBookings: 100
oversoldCount: 0
```

Drain polling 덕분에 spread에서도 정합성이 유지된다.

그런데 total 지표는 3단계와 사실상 구분이 안 된다. spread 처리량 15,559 req/s, total p95 51.7ms는 3단계(약 15,480 req/s, 51.5ms)와 거의 동일하다. 이유는 부하의 99.97%가 패자(409)라 승자 write를 아무리 비동기로 처리해도 total에는 영향을 사실상 미치지 않기 때문이다. 병목은 패자 경로(Redis 직렬화)에 있지 승자 write에 있지 않다. 4단계는 승자 경로만 건드렸는데 승자가 100명뿐이라 total 지표는 그 개선을 못 본다.

4단계에서 크게 개선된 부분은 처리량이 아니라 승자의 200 OK latency다. spread에서 `{ expected_response:true }`(200 OK를 받은 예매자들)의 p95는 1, 2, 3단계 각각 8.8, 9.2, 9.7ms로 8~10ms 수준에 묶여 있었다. 승자가 매 단계 Mongo write를 동기로 기다렸기 때문이다. 4단계에서 그 write를 worker pool로 빼자 1.7ms로 크게 감소했다.
