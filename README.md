...
seatCount: 1
oversoldCount: 1
oversoldSeats: [{ number: 1, bookingCount: 18 }]
```

좌석 하나에 18건의 예매가 기록되는 상당한 oversell이 일어났다. 482건은 정상적으로 409 Conflict를 받았지만, 18건이 동시에 검사를 통과해버린 것이다.
...
oversoldCount: 1 (좌석 1개가 2건 중복)
```

경합을 100개 좌석으로 분산시키자 oversell이 훨씬 드물게 일어났다. Hotspot에서는 요청을 500개밖에 안 날렸는데도 한 좌석에 18건이 중복됐지만, spread에서는 34만 건이 넘는 요청 중 단 1개 좌석만 2번 중복됐다. 경합이 한 점에 몰리느냐 흩어지느냐에 따라 발현 빈도가 크게 달라진다는 걸 보여준다.

이 수치는 이후 단계의 기준선이다. 2단계에서 원자적 연산을 적용해 정합성을 지키고, 3/4단계에서 성능을 올리며 벤치마크가 어떻게 달라지는지 분석할 것이다.
